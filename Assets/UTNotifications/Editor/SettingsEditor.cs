    ...
    //private
        ...
        private void LoadGoogleServicesJson()
        {
            ...
            if (!string.IsNullOrEmpty(filePath))
            {
                ...
                //Let's validate json. If there is an exception in a next line, please make sure you use a valid google-services.json.
                string packageName = json["client"][0]["client_info"]["android_client_info"]["package_name"];
                if (packageName != PlayerSettings.bundleIdentifier)
                {
                    Debug.LogError(string.Format("package_name mismatch! {0}'s package_name = \"{1}\", though Unity app's bundle identifier is \"{2}\"", Path.GetFileName(filePath), packageName, PlayerSettings.bundleIdentifier));
                    return;
                }
