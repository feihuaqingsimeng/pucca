...
public partial class Guild
{
    ...
    public bool IsSupportableRequest(CGuildRequestCard guildRequestCard, out Result_Define.eResult result)
    {
        ...
        if (guildRequestCard != null)
        {
            ...
            {
                int maxCount = 0;
                int guildLevel = Kernel.entry.guild.guildLevel;
                DB_Card.Schema card = DB_Card.Query(DB_Card.Field.Index, guildRequestCard.m_iRequestCardIndex);

                if(card != null)
                {
                    DB_GuildCardRequest.Schema cardSupport = DB_GuildCardRequest.Query(DB_GuildCardRequest.Field.GulidLevel, guildLevel, DB_GuildCardRequest.Field.Grade_Type, card.Grade_Type);

                    if(cardSupport != null)
                        maxCount = cardSupport.Donation_limit;
                }
    ...
    {
        var totalRequestCount = packet.m_iTotalRequestCount;
